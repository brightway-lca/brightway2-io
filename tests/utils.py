...
class UtilsTestCase(unittest.TestCase):
    def test_load_json_data(self):
        self.assertEqual({"1": {"2": 3}}, load_json_data_file("test"))
        self.assertEqual({"1": {"2": 3}}, load_json_data_file("test.json"))

    def test_activity_hash(self):
        self.assertEqual(activity_hash({}), "d41d8cd98f00b204e9800998ecf8427e")
        self.assertEqual(activity_hash({}), "d41d8cd98f00b204e9800998ecf8427e")
        self.assertTrue(isinstance(activity_hash({}), str))
        ds = {"name": "care bears", "unit": "kilogram", "location": "GLO"}
        self.assertEqual(activity_hash(ds), "a6d6dd46cc33acd23826fa5b4e83377f")
        ds = {
            "name": "care bears",
            "categories": ["toys", "fun"],
            "unit": "kilogram",
    ...
    def test_format_for_logging(self):
        ...
        if sys.version_info < (3, 0):
            answer = (
                b"{ u'location': u'GLO', u'name': u'care bears', u'unit': u'kilogram'}"
            )
        else:
            answer = "{'location': 'GLO', 'name': 'care bears', 'unit': 'kilogram'}"
        self.assertEqual(format_for_logging(ds), answer)
