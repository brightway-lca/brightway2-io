...
from bw2data import Database, config, labels, databases
...
class SimaProBlockCSVImporter(LCIImporter):
    ...
    def create_technosphere_placeholders(self, database_name: str):
        ...
        def reformat(exc):
            new_exc = exc | {"type": labels.process_node_default, "exchanges": [], "database": database_name, "code": activity_hash(exc)}
        ...
        proc_data = {(ds["database"], ds["code"]): ds for ds in [
            reformat(exc)
            for ds in self.data
        ...
        if not proc_data:
            print("Skipping placeholder database creation as all technosphere flows are linked")
            return

        print(f"Creating new placeholder database {database_name} with {len(proc_data)} processes")

        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            new_db = Database(database_name)
            new_db.register(
                format=self.format, comment=f"Database for unlinked technosphere flows from {self.db_name}"
            )
    ...
    def use_ecoinvent_strategies(self) -> None:
        ...
        self.strategies = [
            ...
            functools.partial(
                link_iterable_by_fields,
                other=Database(self.default_biosphere_database_name or config.biosphere),
                kind="biosphere",
            ),
        ]

    def write_database(self, backend: Optional[str] = None, activate_parameters: bool = True, searchable: bool = True) -> Database:
        if activate_parameters:
            self.write_project_parameters(delete_existing=False)
        return super().write_database(backend=backend, activate_parameters=activate_parameters, searchable=searchable)
