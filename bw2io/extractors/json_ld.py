...
def walk_dir(dirpath):
    return [(os.path.splitext(file)[0], os.path.join(dirpath, file))
    ...
    def extract(cls, filepath):
        ...
        if os.path.isfile(filepath):
            if not os.path.split(filepath)[1].endswith(".zip"):
                ...
                raise NotImplementedError("Extraction of zip archives not yet supported")
        subdirectories = [os.path.join(filepath, o)
