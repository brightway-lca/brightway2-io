...
- `csv_restore_temporal_distributions(data)`
  ...
  - For each exchange with `temporal_distribution` in `{delta, abs}`:
    ...
    - Require `value` to be floats and rescale if their sum is not 1.0 (divide by sum).
...
```
name, temporal_distribution, date, value, resolution
...
- If `date`/`value` are not list/tuple-like after `csv_restore_tuples`, the strategy should fail fast with a `StrategyError`.
